...
async function loadVirtualModule(id) {
	switch (id) {
		case VIRTUAL_MODULES.startManifest:
			return await import('./assets/_tanstack-start-manifest_v-BLDiog9u.js');
		case VIRTUAL_MODULES.serverFnManifest:
			return await import(
				'./assets/_tanstack-start-server-fn-manifest_v-DTynCcof.js'
			);
		case VIRTUAL_MODULES.injectedHeadScripts:
			return await import(
				'./assets/_tanstack-start-injected-head-scripts_v-cda0Ky0D.js'
			);
		default:
...
function createStartHandler(cb) {
	const APP_BASE = '/';
	const serverFnBase = joinPaths([APP_BASE, trimPath('/_serverFn'), '/']);
	let startRoutesManifest = null;
	let startEntry = null;
	let routerEntry = null;
	const getEntries = async () => {
		if (routerEntry === null) {
			routerEntry = await import('./assets/router-DFuQ1KgV.js').then(
				(n) => n.r,
			);
		}
		if (startEntry === null) {
				...
				if (handler) {
					...
					if (typeof handler === 'function') {
						...
						if (middleware && middleware.length) {
							middlewares.push(
								...flattenMiddlewares(middleware).map(
...
export { createServerRpc as a, createServerFn as c, server as default };
